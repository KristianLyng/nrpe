...
void wait_for_connections(void)
{
	...
	while (1) {
		...
		while (1) {
			...
			retval =
			    select(sock + 1, &fdread, NULL, &fdread, &timeout);
		...
		if (pid == 0) {
			...
			if (pid == 0) {
				...
				request_init(&req, RQ_DAEMON, "nrpe", RQ_FILE,
					     new_sd, 0);
				fromhost(&req);
...
void handle_connection(int sock)
{
	...
	if (result == STATE_OK && use_ssl == TRUE) {
		if ((ssl = SSL_new(ctx)) != NULL) {
			...
			if (rc != 1) {
				syslog(LOG_ERR,
				       "Error: Could not complete SSL handshake. %d\n",
				       SSL_get_error(ssl, rc));
#ifdef DEBUG
	...
	if (use_ssl == FALSE)
		rc = recvall(sock, (char *)&receive_packet, &bytes_to_recv,
			     socket_timeout);
#ifdef HAVE_SSL
	else {
		while (((rc =
			 SSL_read(ssl, &receive_packet, bytes_to_recv)) <= 0)
	...
	/* we couldn't read the correct amount of data, so bail out */
		...
		/* log error to syslog facility */
		syslog(LOG_ERR,
		       "Data packet from client was too short, bailing out...");
	...
	/* find the command we're supposed to run */
		...
		if (temp_command == NULL) {
			...
			if (debug == TRUE)
				syslog(LOG_DEBUG,
				       "Command completed with return code %d and output: %s",
				       result, buffer);
			...
			if ((result < 0) || (result > 3)) {
				...
				/* log error to syslog facility */
				syslog(LOG_ERR, "Bad return code for [%s]: %d",
				       buffer, result);
	...
	/* log info to syslog facility */
	if (debug == TRUE)
		syslog(LOG_DEBUG, "Return Code: %d, Output: %s", result,
		       buffer);
	...
	*early_timeout = FALSE;
	...
	if (pid == 0) {
		...
		if (fp == NULL) {
			...
			while ((bytes_read =
				fread(buffer, 1, sizeof(buffer) - 1, fp)) > 0) {
	...
	/* parent waits for child to finish executing command */
		...
		if (output != NULL) {
			do {
				bytes_read =
				    read(fd[0], output, output_length - 1);
		...
		if (result == STATE_CRITICAL && bytes_read == -1
...
void sighandler(int sig)
{
  ...
  "PROF", "WINCH", "IO", "PWR", "UNUSED", "ZERR", "DEBUG", (char *)NULL };
	int i;
